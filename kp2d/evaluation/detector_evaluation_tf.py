...
from kp2d.evaluation.keypoints_tf import warp_keypoints
...
def compute_repeatability(data, keep_k_points=300, distance_thresh=3):
    ...
    if N1 + N2 > 0 and (count1 + count2) != 0:
        ...
        #print(count1)
        #print(count2)
        #print(data['warped_prob'])
        #print(keep_true_keypoints(data['warped_prob'], np.linalg.inv(H), shape))
        #print(warp_keypoints(data['warped_prob'][:,:2], np.linalg.inv(H)))
        repeatability = -1
        loc_err = -1
    
    #print(data['warped_prob'])
    #print(data['warped_prob'].shape)
    #print(keep_true_keypoints(data['warped_prob'], np.linalg.inv(H), shape).shape)
