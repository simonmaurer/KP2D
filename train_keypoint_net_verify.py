...
from kp2d.evaluation.evaluate_tf import evaluate_step
...
from scripts.train_keypoint_net_utils import (_set_seeds, sample_to_cuda,
...
def main(file):
    ...
    #    summary = None
    if not config.wandb.dry_run:
        summary = SummaryWriter(log_path,config, project=config.wandb.project, entity=config.wandb.entity, job_type='training', mode=os.getenv('WANDB_MODE','run'))
        config.model.checkpoint_path = os.path.join(config.model.checkpoint_path, summary.run_name)
    else:
        summary = None
        date_time = datetime.now().strftime("%m_%d_%Y__%H_%M_%S")
...
def evaluation(config, completed_epoch, model, summary):
    ...
    if True:
        ...
        for params in eval_params:
            ...
            print('PyTorch Repeatability {0:.3f}'.format(rep))
            print('PyTorch Localization Error {0:.3f}'.format(loc))
            print('PyTorch Correctness d1 {:.3f}'.format(c1))
            print('PyTorch Correctness d3 {:.3f}'.format(c3))
            print('PyTorch Correctness d5 {:.3f}'.format(c5))
            print('PyTorch MScore {:.3f}'.format(mscore))

            r, l, c_1, c_3, c_5, m, = [], [], [], [], [], []
            for i, sample in tqdm(enumerate(data_loader), desc="evaluate_keypoint_net_tf"):
                #images, images_transformed, homographies = data
                images = to_color_normalized(sample['image'])
                images_transformed = to_color_normalized(sample['warped_image'])
                homographies = sample['homography']
                scores_ref, positions_ref, descriptors_ref = model_submodule(model).keypoint_net(images)
                scores_transformed, positions_transformed, descriptors_transformed = model_submodule(model).keypoint_net(images_transformed)
                
                scores_ref = tf.constant(scores_ref.permute(0,2,3,1).detach().numpy())
                positions_ref = tf.constant(positions_ref.permute(0,2,3,1).detach().numpy())
                descriptors_ref = tf.constant(descriptors_ref.permute(0,2,3,1).detach().numpy())
                
                scores_transformed = tf.constant(scores_transformed.permute(0,2,3,1).detach().numpy())
                positions_transformed = tf.constant(positions_transformed.permute(0,2,3,1).detach().numpy())
                descriptors_transformed = tf.constant(descriptors_transformed.permute(0,2,3,1).detach().numpy())
                
                r_s, l_s, c_1_s, c_3_s, c_5_s, m_s = evaluate_step(images, images_transformed, homographies, scores_ref, positions_ref, descriptors_ref, scores_transformed, positions_transformed, descriptors_transformed, params['res'], params['top_k'])
            ...
            print('TensorFlow Repeatability {0:.3f}'.format(np.mean(r)))
            print('TensorFlow Localization Error {0:.3f}'.format(np.mean(l)))
            print('TensorFlow Correctness d1 {:.3f}'.format(np.mean(c_1)))
            print('TensorFlow Correctness d3 {:.3f}'.format(np.mean(c_3)))
            print('TensorFlow Correctness d5 {:.3f}'.format(np.mean(c_5)))
            print('Tensorflow MScore {:.3f}'.format(np.mean(m)))
